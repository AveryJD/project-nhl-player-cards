...
def load_stats_csv(season: str, position: str, situation: str) -> pd.DataFrame:
    ...
    stats_file_path = os.path.join(DATA_DIR, 'player_card_data', 'ranking_data', 'stats', stats_file_name)
...
def load_player_ids_csv(season: str) -> pd.DataFrame:
    ...
    player_ids_file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'player_ids', player_ids_file_name)
...
def load_player_bios_csv() -> pd.DataFrame:
    ...
    """
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'player_bios', 'player_bios.csv')
...
def load_shifts_csv(season: str) -> pd.DataFrame:
    ...
    shifts_file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'shifts', shifts_file_name)
...
def load_goals_csv(season: str) -> pd.DataFrame:
    ...
    goals_file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'goals', goals_file_name)
...
def load_shot_events_csv(season: str) -> pd.DataFrame:
    ...
    shot_events_file_path = os.path.join(
        DATA_DIR, 'player_card_data', 'scraped_data', 'shot_events', shot_events_file_name
    )
...
def load_penalty_events_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(
        DATA_DIR, 'player_card_data', 'scraped_data', 'penalty_events', file_name
    )
    return pd.read_csv(file_path)
...
def load_possession_events_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'possession_events', file_name)
    return pd.read_csv(file_path)
...
def load_boxscore_skater_toi_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'boxscore', file_name)
    return pd.read_csv(file_path)
...
def load_faceoffs_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'faceoffs', file_name)
...
def load_schedule_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'schedule', file_name)
...
def load_rapm_scores_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'ranking_data', 'rapm_scores', file_name)

    rapm_scores_df = pd.read_csv(file_path)
    return rapm_scores_df


def load_skater_war_scores_csv(season: str) -> pd.DataFrame:
    """
    Load a season's saved skater WAR scores CSV.

    :param season: A str representing the season ('YYYY-YYYY')
    :return: The DataFrame containing the season's WAR scores.
    """
    file_name = f'{season}_war_scores.csv'
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'ranking_data', 'war_scores', file_name)
...
def load_goalie_war_scores_csv(season: str) -> pd.DataFrame:
    ...
    """
    file_name = f'{season}_goalie_war_scores.csv'
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'ranking_data', 'war_scores', file_name)
    return pd.read_csv(file_path)
...
def load_teammate_toi_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'ranking_data', 'shift_toi', file_name)
...
def load_team_standings_csv(season: str) -> pd.DataFrame:
    ...
    file_path = os.path.join(DATA_DIR, 'player_card_data', 'scraped_data', 'team_standings', file_name)
